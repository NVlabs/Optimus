...
from robosuite.models.objects import BoxObjectWithSites
...
class Stack(SingleArmEnv):
    ...
    def _setup_references(self):
        ...
        self.cubeA_body_id = self.sim.body_name2id(self.cubeA.root_body)
        self.cubeB_body_id = self.sim.body_name2id(self.cubeB.root_body)
...
class StackThree(Stack):
    ...
    def _setup_references(self):
        ...
        # Additional object references from this env
        self.cubeC_body_id = self.sim.body_name2id(self.cubeC.root_body)
...
class StackFour(StackThree):
    ...
    def _setup_references(self):
        ...
        # Additional object references from this env
        self.cubeC_body_id = self.sim.body_name2id(self.cubeC.root_body)
        self.cubeD_body_id = self.sim.body_name2id(self.cubeD.root_body)
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            def gripper_to_cubeD(obs_cache):
                return obs_cache["cubeD_pos"] - obs_cache[f"{pf}eef_pos"] if \
                    "cubeD_pos" in obs_cache and f"{pf}eef_pos" in obs_cache else np.zeros(3)

            @sensor(modality=modality)
            def cubeA_to_cubeD(obs_cache):
                return obs_cache["cubeD_pos"] - obs_cache["cubeA_pos"] if \
                    "cubeA_pos" in obs_cache and "cubeD_pos" in obs_cache else np.zeros(3)

            @sensor(modality=modality)
            def cubeB_to_cubeD(obs_cache):
                return obs_cache["cubeD_pos"] - obs_cache["cubeB_pos"] if \
                    "cubeB_pos" in obs_cache and "cubeD_pos" in obs_cache else np.zeros(3)

            @sensor(modality=modality)
            def cubeC_to_cubeD(obs_cache):
                return obs_cache["cubeD_pos"] - obs_cache["cubeC_pos"] if \
                    "cubeC_pos" in obs_cache and "cubeD_pos" in obs_cache else np.zeros(3)

            sensors = [cubeD_pos, cubeD_quat, gripper_to_cubeD, cubeA_to_cubeD, cubeB_to_cubeD, cubeC_to_cubeD]
            names = [s.__name__ for s in sensors]
...
class StackFive(StackFour):
    ...
    def staged_rewards(self):
        ...
        r_lift = 0.
        r_stack = 0.
        if self._check_cubeA_stacked() and self._check_cubeC_stacked() and self._check_cubeD_stacked() and self._check_cubeE_stacked():
            r_stack = 1.0
    ...
    def _setup_references(self):
        ...
        # Additional object references from this env
        self.cubeE_body_id = self.sim.body_name2id(self.cubeE.root_body)
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            def gripper_to_cubeE(obs_cache):
                return obs_cache["cubeE_pos"] - obs_cache[f"{pf}eef_pos"] if \
                    "cubeE_pos" in obs_cache and f"{pf}eef_pos" in obs_cache else np.zeros(3)

            @sensor(modality=modality)
            def cubeA_to_cubeE(obs_cache):
                return obs_cache["cubeE_pos"] - obs_cache["cubeA_pos"] if \
                    "cubeA_pos" in obs_cache and "cubeE_pos" in obs_cache else np.zeros(3)

            @sensor(modality=modality)
            def cubeB_to_cubeE(obs_cache):
                return obs_cache["cubeE_pos"] - obs_cache["cubeB_pos"] if \
                    "cubeB_pos" in obs_cache and "cubeE_pos" in obs_cache else np.zeros(3)

            @sensor(modality=modality)
            def cubeC_to_cubeE(obs_cache):
                return obs_cache["cubeE_pos"] - obs_cache["cubeC_pos"] if \
                    "cubeC_pos" in obs_cache and "cubeE_pos" in obs_cache else np.zeros(3)

            @sensor(modality=modality)
            def cubeD_to_cubeE(obs_cache):
                return obs_cache["cubeE_pos"] - obs_cache["cubeD_pos"] if \
                    "cubeD_pos" in obs_cache and "cubeE_pos" in obs_cache else np.zeros(3)

            sensors = [cubeE_pos, cubeE_quat, gripper_to_cubeE, cubeA_to_cubeE, cubeB_to_cubeE, cubeC_to_cubeE, cubeD_to_cubeE]
            names = [s.__name__ for s in sensors]
...
class StackFourPickPlace(StackFour):
    ...
    def _load_model(self):
        ...
        self.placement_initializer.reset()
        self.placement_initializer.add_objects_to_sampler(sampler_name="CubesSampler", mujoco_objects=cubes)
        self.placement_initializer.add_objects_to_sampler(sampler_name="baseASampler", mujoco_objects=self.baseA)
        self.placement_initializer.add_objects_to_sampler(sampler_name="baseBSampler", mujoco_objects=self.baseB)
        self.placement_initializer.add_objects_to_sampler(sampler_name="baseCSampler", mujoco_objects=self.baseC)
        self.placement_initializer.add_objects_to_sampler(sampler_name="baseDSampler", mujoco_objects=self.baseD)
