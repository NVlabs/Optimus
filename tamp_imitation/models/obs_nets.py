...
from tamp_imitation.models.base_nets import *
...
class NDPObservationDecoder(Module):
    """
    ...
    is generated with an NDP layer from these flat inputs. Subclass this
    ...
    def __init__(
        ...
        """
        super(NDPObservationDecoder, self).__init__()
        ...
        self.ndp_decoder_kwargs = ndp_decoder_kwargs
        self.single_ndp = ndp_decoder_kwargs.pop("single_ndp", False)
        self._create_layers()
    ...
    def _create_layers(self):
        ...
        for k in self.obs_shapes:
            ...
            if self.single_ndp:
                self.nets[k] = NDPSingle(
                    input_feature_dim=self.input_feat_dim,
                    **self.ndp_decoder_kwargs,
                )
            else:
                self.nets[k] = NDP(
                    input_feature_dim=self.input_feat_dim,
    ...
    def forward(self, inputs):
        ...
        inputs should be a dict containing the input features and the proprio
        """
        output = {}
        for k in self.obs_shapes:
            out = self.nets[k](inputs)
