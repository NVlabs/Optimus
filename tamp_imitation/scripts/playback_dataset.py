...
def playback_trajectory_with_env(
    ...
    """
    assert isinstance(env, EnvBase) or isinstance(env.env, EnvBase)
    ...
    traj_len = len(states) if not action_playback else len(actions)
    if action_playback:
        assert len(states) == (actions.shape[0] + 1)
    ...
    for i in range(traj_len):
        if action_playback:
            info = env.step(
                ...
                # render_intermediate_video_obs=write_video
                # and (env.execute_controller_actions or env.execute_simplified_actions),
            )[-1]
            if i < traj_len - 1:
    ...
    for ind in tqdm(range(len(demos))):
        ...
        if is_robosuite_env:
            ...
            try:
                ...
                print(initial_state["goal_parts_string"])
            except:
                print("No init_string or goal_parts_string in file")
                pass
        ...
        print("trajectory_length: {}".format(len(states)))
        if not success["task"]:
            print("Episode {} failed".format(ep))
            failed_episodes.append(ep)
    print("Failed episodes: {}".format(failed_episodes))
