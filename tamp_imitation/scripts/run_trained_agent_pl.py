...
from tamp_imitation.scripts.pl_train import ModelWrapper
...
def rollout(
    ...
    obs = env.reset(use_eval_indices=True)
    ...
    try:
        for step_i in range(horizon):
            ...
            next_obs, r, done, _ = env.step(
                act[0],
                # render_intermediate_obs=render
                # and (env.execute_controller_actions or env.execute_simplified_actions),
            )
            ...
            if video_writer is not None:
                if video_count % video_skip == 0:
                    ...
                    for cam_name in camera_names:
                        video_img.append(
                            env.render(
                                camera_name=cam_name, mode="rgb_array", height=512, width=512
                            )
                        )
...
def run_trained_agent(args):
    ...
    if config.experiment.rollout.enabled:
        ...
        parent_pid = random.randint(0, 100000000000)
        if "namespace_args" in env_meta["env_kwargs"]:
            task_name = env_meta["env_kwargs"]["namespace_args"]["task_name"]
            scene_path, _ = create_stamp(
                scene_type=task_name if task_name is not None else "table_bins_shapenet",
                seed=0,  # NOTE: never change the STAMP seed!
                stamp_kwargs=env_meta["env_kwargs"]["namespace_args"]["stamp_kwargs"],
            )
            env_meta["env_kwargs"]["namespace_args"]["usd"] = scene_path
            env_meta["env_kwargs"]["namespace_args"]["stamp_kwargs"]["use_usd"] = True
        os.makedirs(os.path.dirname(env_meta_file), exist_ok=True)
        json.dump(env_meta, open(env_meta_file, "w"))
        for env_name in env_names:
            env = TrainUtils.make_parallel_env(
    ...
    if config.algo.transformer.language_enabled:
        ...
        tokenizer, language_token_max_length = (
            trainset.tokenizer,
            trainset.language_token_max_length,
        )
        env.setup_tokenizer(tokenizer, language_token_max_length)
