...
def load_dataset_in_memory(
    ...
    for ep in tqdm(demo_list):
        ...
        traj["obs"]["goal_idx"] = np.concatenate(
            (
                np.zeros(
                    (
                        traj["obs"]["eef_pos"].shape[0]
                        - low_dim_file["data/{}/obs/goal_idx".format(ep)][()].shape[0],
                        1,
                    )
                ),
                low_dim_file["data/{}/obs/goal_idx".format(ep)][()],
            )
        )
...
def combine_hdf5(hdf5_paths, hdf5_use_swmr, dataset_path, low_dim_file):
    ...
    for hdf5_path in tqdm(hdf5_paths):
        try:
            ...
            low_dim_file = h5py.File(low_dim_file, "r", swmr=hdf5_use_swmr, libver="latest")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--low_dim_file")
    parser.add_argument("--output_path")

    args = parser.parse_args()
    combine_hdf5(args.hdf5_paths, True, args.output_path, args.low_dim_file)
